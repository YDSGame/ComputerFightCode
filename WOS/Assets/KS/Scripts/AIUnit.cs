...
public class AIUnit : MonoBehaviour {
    ...
    void SendDead(int _ID, Vector3 _Setposition) //모두에게 죽음을 알림
    {
        ...
        AIUnit unitAI = deadUnit.GetComponent<AIUnit>();
        UnitState unitSt = deadUnit.GetComponent<UnitState>();
        unitAI.healthUI.gameObject.SetActive(false); //찾은 놈의 체력바를 숨김
        deadUnit.GetComponent<CapsuleCollider>().enabled = false;
        deadUnit.transform.position = _Setposition;         //리스폰 될 지점을 세팅해줌    
        unitAI.myRigidBody.isKinematic = false;
        unitSt.pHealth = unitSt.pMaxHealth;
        unitAI.myActive.SetActive(false);        
    }
    ...
    {       
        ...
        //    }
        //}
        cam = Camera.main.transform; //체력바가 바라봐야할 카메라
        //healthUI.gameObject.SetActive(true); //체력바 켜줌
        
        healthUI.transform.forward = -cam.forward; //체력바가 카메라를 바라봄      
        wayPoint = wayPoints[0]; //웨이포인트 초기화
        myAudioSource.clip = mySoundAndEffect.attackSoundClip; //공격 사운드 받아오기
    }
    ...
    public IEnumerator Action() //행동
    {
        while (myActive.activeInHierarchy)
        {
            ...
            healthSlider.fillAmount = myState.pHealth / myState.pMaxHealth; //체력바 갱신
                ...
                case UnitState.eState.Dead: //죽음 상태
                    {
                        ...
                        target = null;
                        healthUI.gameObject.SetActive(false); //체력바 끄기
