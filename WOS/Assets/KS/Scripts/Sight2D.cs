...
public class Sight2D : MonoBehaviour {
    ...
    private List<Collider> hitedTargetContainers = new List<Collider>();
    ...
    public Collider[] FindViewTargets()
    {
        ...
        Collider[] hitedTargets = Physics.OverlapSphere(originPos, m_viewRadius, m_viewTargetMask);

        foreach (Collider hitedTarget in hitedTargets)
        {
            ...
            if (angle <= m_horizontalViewHalfAngle)
            {
                ...
                {
                    print("안들어옴");
                    hitedTargetContainers.Add(hitedTarget);
        ...
        if (hitedTargetContainer.Count > 0)
            return hitedTargetContainers.ToArray();
        else
            return null;
    }
